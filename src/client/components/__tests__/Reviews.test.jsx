...
describe('Review component', () => {
  ...
  test('it should be selectable by class "review"', () => {
    expect(shallow(<Reviews />).is('.review')).toBe(true);
  });

  test('it should mount in a full DOM', () => {
    expect(mount(<Reviews />).find('.review').length).toBe(1);
  });

  test('it should render to static HTML', () => {
    expect(render(<Reviews />).text()).toEqual('Review from ChrisThis is a review summary');
  });
});
