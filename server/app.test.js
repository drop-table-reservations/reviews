...
describe('GET /restaurants/:restaurantId/reviews', () => {
  test('it should respond with 200 for existing restaurant', async () => {
    const res = await request(app).get('/restaurants/12345678/reviews');
    expect(res.statusCode).toBe(200);
  });

  test('it should respond with 404 for non-existent restaurant', async () => {
    const res = await request(app).get('/restaurants/99999999/reviews');
  ...
  test('it should respond with empty array if no reviews', async () => {
    const res = await request(app).get('/restaurants/87654321/reviews');
